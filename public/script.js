...
async function startTask(taskNum) {
    ...
    if (taskNum === 4 || taskNum === 5) { // <--- MODIFIED HERE: Add taskNum === 5
        ...
        try {
            ...
            console.error("Error gathering summaries for Task 4/5:", err); // <--- MODIFIED HERE: Error message update
            chatBox.innerText = "❌ Failed to gather previous task information. Starting task without full context."; // <--- MODIFIED HERE: Error message update
